...
if option == "A": #requirement installation
    ...
    if insnmap in ['Y','N']:
        ...
        if insnmap == "Y":
            subprocess.run(['curl','https://nmap.org/dist/nmap-7.91-setup.exe','-o','setups/nmap-7.91-setup.exe'])
            subprocess.Popen(['setups/nmap-7.91-setup.exe'])
            subprocess.run(['python','choices.py'])
    ...
    subprocess.run(['python', 'tools/port-finder.py'])
